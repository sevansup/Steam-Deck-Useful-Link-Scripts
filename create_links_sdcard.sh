...
internal_dir="/home/deck/.local/share/Steam/steamapps"
base_dir="/run/media/mmcblk0p1/steamapps"
app_mani_dir="${base_dir}/appmanifest*.acf"
compat_dir="${internal_dir}/compatdata"
...
if [[ ! -L "$shortcuts_directory/compatdata sd" ]]; then
  ln -s ${compat_dir} "$shortcuts_directory/compatdata sd"
  echo "Created compatdata shortcut: $shortcuts_directory/compatdata sd"
  # Spacer
  echo
fi

# Create empty App ID file
echo "" 1>"$shortcuts_directory/app_ids sdcard.txt" 2>/dev/null
...
for file in ${app_mani_dir}; do
  ...
  if [[ -d "$app_dir" ]]; then
    ...
    echo "App Name: $app_name | App ID: $app_id | $compat_dir/$app_id" >>"$shortcuts_directory/app_ids sdcard.txt"
    ...
    echo "App Name: $app_name | App ID: $app_id | (Does Not Have a Folder)" >>"$shortcuts_directory/app_ids sdcard.txt"
